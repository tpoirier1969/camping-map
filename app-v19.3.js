...
function summaryMarkerHtml(counts = {}) {
  return `
    <div class="summary-marker-row">
      ${summaryBadgeHtml('state', BUILTIN_BUCKETS.state.color, counts.campgrounds || 0)}
...
function renderSummaryMarkers() {
  ...
  for (const feature of fc.features) {
    ...
    el.innerHTML = `${props.state ? `<div class="summary-state-chip">${escapeHtml(props.state || '')}</div>` : ''}${summaryMarkerHtml(props)}`;
