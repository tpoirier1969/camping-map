...
const BUILTIN_BUCKETS = {
  modern: { label: 'Modern', color: '#6b8e23', radius: 13 },
  rustic: { label: 'Rustic', color: '#8b5a2b', radius: 13 },
  boondocking: { label: 'Boondocking / dispersed', color: '#2f6f3e', radius: 13 },
  private: { label: 'Private campgrounds', color: '#8a4f2d', radius: 13 },
  federal: { label: 'Federal campgrounds', color: '#8c5a22', radius: 13 },
  state: { label: 'State campgrounds', color: '#5f7f2b', radius: 13 },
  local: { label: 'Local campgrounds', color: '#b56a2d', radius: 13 },
  national_forest: { label: 'National forest campgrounds', color: '#7b5a22', radius: 13 },
  state_federal_modern: { label: 'State / federal modern campgrounds', color: '#6b8e23', radius: 13 },
  state_federal_rustic: { label: 'State / federal rustic campgrounds', color: '#8b5a2b', radius: 11 },
  state_local: { label: 'State / local campgrounds', color: '#b56a2d', radius: 13 },
  trailhead: { label: 'Trailheads', color: '#6c7a4a', radius: 13 },
  info: { label: 'Info / reference', color: '#5b6e57', radius: 11 },
  other: { label: 'Other campsites', color: '#7c6f54', radius: 13 },
  state_summary: { label: 'State summary', color: '#735135', radius: 24 },
...
function symbolSvg(symbol, color = 'currentColor') {
  const fill = color;
  const stroke = 'rgba(18, 12, 7, 0.95)';
  switch (symbol) {
    case 'tree':
      return `<svg viewBox="0 0 64 64" aria-hidden="true"><path d="M32 6 18 24h8L15 39h10l-6 11h26l-6-11h10L38 24h8L32 6Z" fill="${fill}" stroke="${stroke}" stroke-width="3" stroke-linejoin="round"/><path d="M32 49v9" stroke="${stroke}" stroke-width="4" stroke-linecap="round"/></svg>`;
    case 'arrowhead':
      return `<svg viewBox="0 0 64 64" aria-hidden="true"><path d="M32 5C22 15 16 28 18 44c6 2 10 7 14 15 4-8 8-13 14-15 2-16-4-29-14-39Z" fill="${fill}" stroke="${stroke}" stroke-width="3" stroke-linejoin="round"/><path d="M24 25h16M28 33h8" stroke="#f6ead3" stroke-width="3.2" stroke-linecap="round"/></svg>`;
    case 'tent':
      return `<svg viewBox="0 0 64 64" aria-hidden="true"><path d="M9 48 32 13l23 35H9Z" fill="${fill}" stroke="${stroke}" stroke-width="3" stroke-linejoin="round"/><path d="M32 13v35" stroke="#f8edd6" stroke-width="3" stroke-linecap="round"/><path d="M20 48 32 30l12 18" stroke="#f8edd6" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/></svg>`;
    case 'campfire':
      return `<svg viewBox="0 0 64 64" aria-hidden="true"><path d="M33 10c7 8 9 14 3 21 9-1 14 5 14 12 0 9-7 15-17 15s-17-6-17-15c0-8 5-14 13-17-3-5-2-10 4-16Z" fill="${fill}" stroke="${stroke}" stroke-width="3" stroke-linejoin="round"/><path d="M19 54 45 46M19 46l26 8" stroke="#f7ecd7" stroke-width="3.2" stroke-linecap="round"/></svg>`;
    case 'camper':
      return `<svg viewBox="0 0 64 64" aria-hidden="true"><path d="M10 25h28l10 8h6v13H10Z" fill="${fill}" stroke="${stroke}" stroke-width="3" stroke-linejoin="round"/><circle cx="22" cy="46" r="5" fill="#f6ead3" stroke="${stroke}" stroke-width="3"/><circle cx="45" cy="46" r="5" fill="#f6ead3" stroke="${stroke}" stroke-width="3"/><path d="M18 30h12v8H18Z" fill="#f6ead3" stroke="${stroke}" stroke-width="2.4"/></svg>`;
    case 'info':
      return `<svg viewBox="0 0 64 64" aria-hidden="true"><circle cx="32" cy="32" r="22" fill="${fill}" stroke="${stroke}" stroke-width="3"/><path d="M32 27v17" stroke="#f7edd9" stroke-width="4" stroke-linecap="round"/><circle cx="32" cy="19.5" r="3.2" fill="#f7edd9"/></svg>`;
    case 'trail':
      return `<svg viewBox="0 0 64 64" aria-hidden="true"><path d="M18 54V18m28 36V18" stroke="${stroke}" stroke-width="4" stroke-linecap="round"/><path d="M18 19h17l11 8H29Z" fill="${fill}" stroke="${stroke}" stroke-width="3" stroke-linejoin="round"/></svg>`;
    default:
      return `<svg viewBox="0 0 64 64" aria-hidden="true"><circle cx="32" cy="32" r="18" fill="${fill}" stroke="${stroke}" stroke-width="3"/></svg>`;
  }
}
...
function shouldShowSiteDetails() {
  ...
  const focused = Boolean(focusedOnSingleState());
  const visibleStates = visibleStatesInViewport();
  if (zoom >= 7.6) return true;
  if (focused && zoom >= 6.2) return true;
  if (visibleStates.length <= 2 && zoom >= 6.8) return true;
  return false;
}
...
function desiredSummaryCountForState(sites = []) {
  if (sites.length >= 100) return 4;
  if (sites.length >= 35) return 3;
  if (sites.length >= 8) return 2;
  return 1;
}
...
function summaryGroupsForState(sites = []) {
  ...
  const cellSize = zoom <= 2.5 ? 3.4 : zoom <= 4 ? 2.4 : 1.5;
  ...
  if (groups.length < desired && sites.length >= desired * 3) groups = splitSitesByAxis(sites, desired);
  if (!groups.length && sites.length) groups = [sites];
  return groups;
}
...
function renderDirectSiteMarkers() {
  ...
  for (const site of visibleSites) {
    ...
    el.style.filter = 'drop-shadow(0 0 2px rgba(255,248,232,0.7)) drop-shadow(0 1px 3px rgba(0,0,0,0.9))';
    el.style.margin = '0';
