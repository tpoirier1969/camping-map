...
async function loadData() {
  ...
  const [sitesRaw, trailRaw] = await Promise.all([
...
function renderDirectSiteMarkers() {
  ...
  const visibleSites = enabledSites()
    .filter((site) => Array.isArray(site.lngLat) && site.lngLat.length >= 2 && bounds.contains(site.lngLat))
    .slice(0, 350);
  ...
  for (const site of visibleSites) {
    ...
    el.style.boxShadow = '0 0 0 2px rgba(0,0,0,0.65), 0 2px 8px rgba(0,0,0,0.45)';
    el.style.padding = '0';
    el.style.margin = '0';
