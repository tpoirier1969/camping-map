...
const EXTRA_SITE_DATA_URLS = ['data/sites-additions-v18.5.json'];
const TRAIL_GEOJSON_URLS = ['data/trails.geojson', 'trails.geojson'];
const TRAIL_VECTOR_MANIFEST_URLS = ['data/trails.vector.json', 'trails.vector.json'];
...
const BUILTIN_BUCKETS = {
  ...
  federal: { label: 'Federal campgrounds', color: '#1f8a70', radius: 13 },
  state: { label: 'State campgrounds', color: '#2a7fff', radius: 13 },
  local: { label: 'Local campgrounds', color: '#8e5bd6', radius: 13 },
  ...
  info: { label: 'Info / reference', color: '#8c8c8c', radius: 11 },
...
async function loadTrailData() {
  const vectorManifest = await loadFirstAvailable(TRAIL_VECTOR_MANIFEST_URLS, 'trails');
  if (vectorManifest?.tiles?.length) {
    model.trailSourceMode = 'vector';
    model.trailVectorConfig = {
      sourceLayer: vectorManifest.sourceLayer || 'trails',
      tiles: vectorManifest.tiles,
      minzoom: Number.isFinite(Number(vectorManifest.minzoom)) ? Number(vectorManifest.minzoom) : 0,
      maxzoom: Number.isFinite(Number(vectorManifest.maxzoom)) ? Number(vectorManifest.maxzoom) : 14
    };
    model.trails = null;
    return;
  }
  const trailGeoJson = await loadFirstAvailable(TRAIL_GEOJSON_URLS, 'trails');
  const features = Array.isArray(trailGeoJson?.features) ? trailGeoJson.features.map(normalizeTrailFeature) : [];
  model.trails = features.length ? { type: 'FeatureCollection', features } : null;
  model.trailSourceMode = features.length ? 'geojson' : 'none';
  model.trailVectorConfig = null;
}

function trailSourceLoaded() {
  return model.trailSourceMode === 'vector' ? Boolean(model.trailVectorConfig?.tiles?.length) : Boolean(model.trails?.features?.length);
}
...
function symbolSvg(symbol, stroke = '#ffffff') {
  switch (symbol) {
    case 'tree':
      return `<svg viewBox="0 0 64 64" aria-hidden="true"><path d="M32 7 18 25h7L14 40h11l-4 9h22l-4-9h11L39 25h7L32 7Z" fill="none" stroke="${stroke}" stroke-width="4" stroke-linejoin="round"/><path d="M32 49v9" stroke="${stroke}" stroke-width="4" stroke-linecap="round"/></svg>`;
    case 'arrowhead':
      return `<svg viewBox="0 0 64 64" aria-hidden="true"><path d="M32 6C22 16 17 29 18 44c7 2 10 7 14 14 4-7 7-12 14-14 1-15-4-28-14-38Z" fill="none" stroke="${stroke}" stroke-width="4" stroke-linejoin="round"/><path d="M23 26h18M27 34h10" stroke="${stroke}" stroke-width="3.5" stroke-linecap="round"/></svg>`;
    case 'tent':
      return `<svg viewBox="0 0 64 64" aria-hidden="true"><path d="M10 46 32 14l22 32H10Z" fill="none" stroke="${stroke}" stroke-width="4" stroke-linejoin="round"/><path d="M32 14v32" stroke="${stroke}" stroke-width="4" stroke-linecap="round"/><path d="M21 46 32 30l11 16" stroke="${stroke}" stroke-width="4" stroke-linecap="round" stroke-linejoin="round"/></svg>`;
    case 'campfire':
      return `<svg viewBox="0 0 64 64" aria-hidden="true"><path d="M32 11c5 8 6 13 1 19 8-2 13 4 13 11 0 8-6 13-14 13s-14-5-14-13c0-7 5-13 12-16-2-4-1-9 2-14Z" fill="none" stroke="${stroke}" stroke-width="4" stroke-linejoin="round"/><path d="M20 54 44 46M20 46l24 8" stroke="${stroke}" stroke-width="4" stroke-linecap="round"/></svg>`;
    case 'camper':
      return `<svg viewBox="0 0 64 64" aria-hidden="true"><path d="M10 23h30l8 8h6v15H10Z" fill="none" stroke="${stroke}" stroke-width="4" stroke-linejoin="round"/><circle cx="22" cy="46" r="5" fill="none" stroke="${stroke}" stroke-width="4"/><circle cx="45" cy="46" r="5" fill="none" stroke="${stroke}" stroke-width="4"/><path d="M18 28h12v8H18Z" fill="none" stroke="${stroke}" stroke-width="3"/></svg>`;
    case 'info':
      return `<svg viewBox="0 0 64 64" aria-hidden="true"><circle cx="32" cy="32" r="22" fill="none" stroke="${stroke}" stroke-width="4"/><path d="M32 28v16" stroke="${stroke}" stroke-width="4" stroke-linecap="round"/><circle cx="32" cy="20" r="2.8" fill="${stroke}"/></svg>`;
    case 'trail':
      return `<svg viewBox="0 0 64 64" aria-hidden="true"><path d="M17 52V18m30 34V18" stroke="${stroke}" stroke-width="4" stroke-linecap="round"/><path d="M17 19h18l12 9H29Z" fill="none" stroke="${stroke}" stroke-width="4" stroke-linejoin="round"/></svg>`;
    default:
      return `<svg viewBox="0 0 64 64" aria-hidden="true"><circle cx="32" cy="32" r="18" fill="none" stroke="${stroke}" stroke-width="4"/></svg>`;
  }
}

function markerPreviewHtml(bucket, color, size = 18) {
  const symbol = bucketSymbol(bucket);
  return `<span class="symbol-preview" style="--preview-size:${size}px;--preview-color:${escapeAttribute(color || '#666')}">${symbolSvg(symbol)}</span>`;
}
...
function stateSummaryLabel(props = {}) {
  ...
  return `${state}
C ${props.campgrounds || 0} · B ${props.boondocking || 0} · I ${props.info || 0}`;
}
...
function renderLayerControls() {
  ...
  for (const def of model.layerDefs.values()) {
    ...
    row.innerHTML = `<input type="checkbox" data-layer-key="${escapeAttribute(def.key)}" checked>
      ${markerPreviewHtml(def.bucket, def.color, 16)}
...
function renderLegend() {
  const items = [{ type: 'dot', label: 'State summary', color: BUILTIN_BUCKETS.state_summary.color, bucket: 'state_summary' }];
  for (const def of model.layerDefs.values()) items.push({ type: 'dot', label: def.label, color: def.color, bucket: def.bucket });
  if (trailSourceLoaded()) items.push({ type: 'line', label: 'Trails overlay', color: BUILTIN_BUCKETS.trail.color });
  if (trailSourceLoaded()) items.push({ type: 'line', label: 'Long-distance trails', color: '#ff7a00' });
  items.push({ type: 'dot', label: 'Draft site', color: BUILTIN_BUCKETS.draft.color, bucket: 'draft' });
  els.legendList.innerHTML = items.map((item) => `<div class="legend-item">${item.type === 'line' ? `<span class="legend-line" style="border-top-color:${escapeAttribute(item.color)}"></span>` : markerPreviewHtml(item.bucket, item.color, 16)}<span>${escapeHtml(item.label)}</span></div>`).join('');
}

function syncTrailUi() {
  const hasTrails = trailSourceLoaded();
  els.trailSection.hidden = !hasTrails;
  const modeLabel = model.trailSourceMode === 'vector' ? 'Vector tiles' : model.trailSourceMode === 'geojson' ? 'GeoJSON' : '';
  els.trailStatusText.textContent = hasTrails ? `${modeLabel} trail overlay loaded.` : 'No trail overlay loaded.';
}
...
function shouldShowSiteDetails() {
  ...
  if (visibleStates.length <= 1 && lngSpan <= 18 && latSpan <= 12) return true;
  if (visibleStates.length <= 3 && lngSpan <= 26 && latSpan <= 16) return true;
  if (visibleStates.length <= 6 && zoom >= 4.8) return true;
  if (visibleStates.length <= 10 && zoom >= 5.0) return true;
  return false;
}
...
function stateCircleRadiusExpression() {
  const sizeByCount = ['interpolate', ['linear'], ['+', ['get', 'campgrounds'], ['get', 'boondocking'], ['get', 'info']], 1, 18, 10, 22, 25, 28, 50, 34, 100, 40];
  return ['*',
    sizeByCount,
    ['case',
      ['<=', ['zoom'], 5], 0.72,
...
function renderDirectSiteMarkers() {
  ...
  const zoom = model.map.getZoom();
  const markerSize = siteMarkerSizeForZoom(zoom) + 6;
  const markerStroke = Math.max(2, siteMarkerStrokeForZoom(zoom) - 0.6);
  ...
  for (const site of visibleSites) {
    ...
    el.style.borderRadius = '999px';
    el.style.border = `${markerStroke}px solid rgba(255,255,255,0.95)`;
    el.style.background = def.color || '#ff2d55';
    el.style.boxShadow = '0 0 0 3px rgba(0,0,0,0.72), 0 2px 10px rgba(0,0,0,0.48)';
    ...
    el.innerHTML = symbolSvg(bucketSymbol(bucket));
...
function setApiKeyUi() {
  ...
  if (els.keySection) {
    els.keySection.hidden = model.hasApiKey;
  }
...
function bindUi() {
  ...
  els.revealKeySectionBtn?.addEventListener('click', () => {
    if (els.keySection) {
      const willShow = els.keySection.hidden;
      els.keySection.hidden = !willShow;
