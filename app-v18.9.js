...
const EXTRA_SITE_DATA_URLS = ['data/sites-additions-v18.5.json', 'data/sites-additions-v18.7.json', 'data/sites-additions-v18.8.json', 'data/sites-additions-v18.9.json'];
...
const BUILTIN_BUCKETS = {
  modern: { label: 'Modern', color: '#5f7f2b', radius: 13 },
  rustic: { label: 'Rustic', color: '#8a5a2b', radius: 13 },
  boondocking: { label: 'Boondocking / dispersed', color: '#2f6b3b', radius: 13 },
  private: { label: 'Private campgrounds', color: '#7d4d5a', radius: 13 },
  federal: { label: 'Federal campgrounds', color: '#9a5c26', radius: 13 },
  state: { label: 'State campgrounds', color: '#627b2f', radius: 13 },
  local: { label: 'Local campgrounds', color: '#b0642f', radius: 13 },
  national_forest: { label: 'National forest campgrounds', color: '#9a5c26', radius: 13 },
  state_federal_modern: { label: 'State / federal modern campgrounds', color: '#627b2f', radius: 13 },
  state_federal_rustic: { label: 'State / federal rustic campgrounds', color: '#8a5a2b', radius: 11 },
  state_local: { label: 'State / local campgrounds', color: '#b0642f', radius: 13 },
  trailhead: { label: 'Trailheads', color: '#6f7b62', radius: 13 },
  info: { label: 'Info / reference', color: '#5e6a69', radius: 11 },
  other: { label: 'Other campsites', color: '#7e7466', radius: 13 },
  state_summary: { label: 'State summary', color: '#7c5832', radius: 24 },
...
function symbolSvg(symbol, stroke = 'currentColor') {
  switch (symbol) {
    case 'tree':
...
function renderLayerControls() {
  ...
  for (const def of model.layerDefs.values()) {
    ...
    row.innerHTML = `<input type="checkbox" data-layer-key="${escapeAttribute(def.key)}" checked>
      ${markerPreviewHtml(def.bucket, def.color, 20)}
...
function renderLegend() {
  ...
  for (const def of [...model.layerDefs.values()]) {
    if (def.bucket === 'trail') continue;
    items.push({ type: 'dot', label: def.label, color: def.color, bucket: def.bucket });
  }
  els.legendList.innerHTML = items.map((item) => `<div class="legend-item">${markerPreviewHtml(item.bucket, item.color, 22)}<span>${escapeHtml(item.label)}</span></div>`).join('');
}
...
function desiredSummaryCountForState(sites = []) {
  if (sites.length >= 150) return 4;
  if (sites.length >= 50) return 3;
...
function summaryGroupsForState(sites = []) {
  ...
  const cellSize = zoom <= 2.5 ? 3.1 : zoom <= 4 ? 2.0 : 1.2;
  const grouped = new Map();
  for (const site of sites) {
    const [lng, lat] = site.lngLat;
    const key = `${Math.floor((lng + 180) / cellSize)}:${Math.floor((lat + 90) / cellSize)}`;
    if (!grouped.has(key)) grouped.set(key, []);
    grouped.get(key).push(site);
  }
  let groups = [...grouped.values()].filter((g) => g.length);
  const desired = desiredSummaryCountForState(sites);
  if (groups.length < desired && sites.length >= desired * 4) groups = splitSitesByAxis(sites, desired);
  return groups;
}
...
function buildStateSummaryGeoJson() {
  ...
  for (const [state, sites] of byState.entries()) {
    ...
    groups.forEach((group, idx) => {
      ...
      const centroid = [group.reduce((a, s) => a + s.lngLat[0], 0) / group.length, group.reduce((a, s) => a + s.lngLat[1], 0) / group.length];
      features.push({
        type: 'Feature',
        properties: {
          state,
...
function stateCircleRadiusExpression() {
  const sizeByCount = ['interpolate', ['linear'], ['+', ['get', 'campgrounds'], ['get', 'boondocking'], ['get', 'info']], 1, 14, 10, 18, 25, 22, 50, 26, 100, 30];
  return ['*',
    sizeByCount,
    ['case',
      ['<=', ['zoom'], 5], 0.72,
