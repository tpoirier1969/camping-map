...
function normalizeCategory(rawCategory = '') {
  ...
  if ((value.includes('state') || value.includes('county') || value.includes('local') || value.includes('municipal') || value.includes('city') || value.includes('town')) && value.includes('camp')) return 'state_local';
  if (value.includes('state') && value.includes('modern')) return 'state_federal_modern';
  if ((value.includes('county') || value.includes('local') || value.includes('municipal') || value.includes('city') || value.includes('town')) && value.includes('camp')) return 'state_local';
...
function deriveLayerInfo(raw, category) {
  ...
  if (layerish) {
    ...
    if (bucket === 'state_local' && label.toLowerCase() === 'public') label = 'State / Local Public';
    if (bucket === 'info') label = 'Info / Reference';
    return { key: makeSlug(label) || `layer-${category}`, label, bucket };
  }
  if (ownerText.includes('private')) return { key: 'private-campgrounds', label: 'Private Campgrounds', bucket: 'private' };
  if (ownerText.includes('county') || ownerText.includes('local') || ownerText.includes('municipal') || ownerText.includes('city') || ownerText.includes('town')) {
    return { key: 'state-local-public', label: 'State / Local Public', bucket: 'state_local' };
  }
  ...
  if (category === 'info') return { key: 'info-reference', label: 'Info / Reference', bucket: 'info' };
  ...
  if (category === 'state_local') return { key: 'state-local-public', label: 'State / Local Public', bucket: 'state_local' };
...
function renderDirectSiteMarkers() {
  clearDomMarkers();
  if (!model.map || !shouldShowSiteDetails()) return;
  let bounds = null;
