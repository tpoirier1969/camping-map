...
function summaryMarkerHtml(counts = {}) {
  return `
    <div class="summary-marker">
      <div class="summary-row">${markerPreviewHtml('state', BUILTIN_BUCKETS.state.color, 18)}<span>${counts.campgrounds || 0}</span></div>
      <div class="summary-row">${markerPreviewHtml('boondocking', BUILTIN_BUCKETS.boondocking.color, 18)}<span>${counts.boondocking || 0}</span></div>
      <div class="summary-row">${markerPreviewHtml('info', BUILTIN_BUCKETS.info.color, 18)}<span>${counts.info || 0}</span></div>
    </div>`;
}
...
function renderSummaryMarkers() {
  ...
  for (const feature of fc.features) {
    ...
    el.innerHTML = `<div class="summary-state">${escapeHtml(props.state || '')}</div>${summaryMarkerHtml(props)}`;
