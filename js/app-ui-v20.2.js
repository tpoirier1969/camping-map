...
function resetToDefaultView() {
  if (!model.map) return;
  model.map.flyTo({ center: DEFAULT_CENTER, zoom: DEFAULT_ZOOM, essential: true, speed: 0.9 });
  setSearchStatus('Reset to default view.');
}

function zoomToUserLocation() {
  if (!model.map || !navigator.geolocation) {
    setSearchStatus('Location is not available on this device.');
    return;
  }
  setSearchStatus('Finding your location…');
  navigator.geolocation.getCurrentPosition((position) => {
    const center = [position.coords.longitude, position.coords.latitude];
    if (model.locateMarker?.remove) model.locateMarker.remove();
    const el = document.createElement('div');
    el.className = 'my-location-marker';
    model.locateMarker = new maptilersdk.Marker({ element: el, anchor: 'center' }).setLngLat(center).addTo(model.map);
    model.map.flyTo({ center, zoom: 10, essential: true, speed: 0.9 });
    setSearchStatus('Centered on your location.');
  }, () => {
    setSearchStatus('Could not get your location. Check location permissions.');
...
function bindUi() {
  ...
  els.resetViewBtn?.addEventListener('click', resetToDefaultView);
  els.locateBtn?.addEventListener('click', zoomToUserLocation);
